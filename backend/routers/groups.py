...
def list_groups(category: str | None = None, db: Session = Depends(get_db)):
    ...
    groups = q.order_by(FileGroup.name).all()

    if not groups:
        # Still need ungrouped count even when there are no named groups
        ...
        return {"groups": [], "ungrouped_count": ungrouped_q.scalar() or 0}

    # Single query: count files per group_id for all groups at once
    ...
    for g in groups:
        d = g.to_dict()
        d["file_count"] = counts.get(g.id, 0)
        result.append(d)

    # Count ungrouped files (single query)
    ungrouped_q = db.query(func.count(FileRecord.id)).filter(
        FileRecord.group_id == None  # noqa: E711
    )
    if category:
