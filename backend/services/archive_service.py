...
from database.models import ArchiveJob, FileGroup, FileRecord, SessionLocal
...
def _archive_worker(group_id: int, archive_dir: str) -> None:
    ...
    try:
        ...
        # ── Step 1: copy SHARED DLLs to shared storage, get back a manifest list ──
        ...
        if shared_dir:
            try:
                ...
                def _dll_scan_progress(pct: int) -> None:
                    # Map 0-100 from scan phase → 2-9% overall
        ...
        # ── Step 2: zip game files (unique DLLs included, shared DLLs excluded) + embed manifest ──
        ...
        total = len(all_files)
        logger.info("Archiving group %d (%s): %d files → %s", group_id, grp.name, total, zip_path)
        ...
        with zipfile.ZipFile(
            ...
            for i, fp in enumerate(all_files):
                ...
                zf.write(fp, arcname)
                # Scale 10% → 90% across the file list
        ...
        logger.info(
            "Archive done for group %d → %s (%.1f MB)", group_id, zip_path, archive_size / 1e6
        )

        # ── Step 5: remove shared DLL zips no longer needed by any active game ──
        if shared_dir:
            try:
...
def _restore_worker(group_id: int) -> None:
    ...
    try:
        ...
        # ── Step 1: read embedded _dlls.json, then extract game files ──
        ...
        # ── Step 2: restore DLLs from shared storage ──
