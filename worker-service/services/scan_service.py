...
# Active scan job id â†’ Future mapping
_active_jobs: dict[int, object] = {}
_lock = threading.Lock()


def _upsert_file_batch(db, values: list[dict]) -> None:
    """Bulk upsert file records â€” insert new, update metadata for existing.
    ...
    max_attempts = 120  # 120 Ă— 0.5 s = 60 s of retry headroom
    for attempt in range(max_attempts):
        try:
            ...
            if ("locked" in err or "busy" in err) and attempt < max_attempts - 1:
                logger.debug(
                    "_upsert_file_batch: DB locked (attempt %d/%d), retrying in 0.5 sâ€¦",
                    attempt + 1,
                    max_attempts,
                )
...
def _run_scan(job_id: int, root_path: str):
    ...
    try:
        ...
        for file_meta in scan_directory(root_path):
            ...
            cat = str(categorize(fp))  # coerce np.str_ â†’ str
        ...
        logger.info("[Job %d] Detecting groups â€¦", job_id)
...
def _run_rescan_all(job_id: int, disks: list[str]):
    ...
    try:
        ...
        # Phase 2 â€” index every disk
        processed = 0
        for disk in disks:
            logger.info("[Job %d] Indexing %s â€¦", job_id, disk)
        ...
        # Phase 3 â€” detect groups for every disk (fast SQL only, no icon extraction)
        for disk in disks:
            try:
        ...
        # Phase 4 â€” icon extraction in background (job already marked done)
        for disk in disks:
            try:
...
def _index_groups(db, root_path: str, extract_icons: bool = True):
    """Build FileGroup records from DB data (no disk walk needed â€” files are
    already committed when this is called).
    Uses regroup_from_db which runs pure SQL â€” fast even for full-disk scans.
