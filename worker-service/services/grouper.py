...
Strategy:
  ...
  2. For each leaf, bubble UP through generic sub-folder names
     (bin, data, scripts, x64, â€¦) until we reach a meaningful
     named folder â€” that becomes the group root.
...
# â”€â”€ Generic sub-folder names â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Folders with these names are internal parts of a larger unit, NOT the unit
# itself â€” so we bubble up past them when searching for the group root.
_GENERIC = {
...
def detect_groups(root: str) -> list[dict]:
    ...
    # Step 1 â€” collect all leaf dirs and their extensions
    leaf_data: dict[str, set[str]] = {}  # path_str â†’ set of extensions
    _collect_leaves(root_path, root_path, leaf_data)

    if not leaf_data:
        return []

    # Step 1b â€” find every folder that directly contains a .exe (game/app anchor),
    # then propagate each anchor UP to its logical game/app root
    # (e.g. GAME\bin\game.exe â†’ GAME, not bin)
    ...
    # Step 2 â€” bubble up each leaf to its group root; merge extensions
    group_exts: dict[str, set[str]] = {}  # group_root_str â†’ all extensions
...
def _collect_leaves(
    ...
    """Recursively populate *result* with leaf dirs â†’ extension sets."""
    if depth > max_depth:
        return
    try:
...
def _find_group_root(
    ...
    # Check whether any segment IS a known game root â€” return it immediately
    for seg in segments:
        if str(seg) in game_roots:
            return seg

    # No game root in path â€” fall back to highest non-generic ancestor
...
# â”€â”€ DB-only re-grouping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def regroup_from_db(db, root_path: str) -> list[dict]:
    ...
    from diskassistent_db.models import FileRecord
    ...
    # 1 â€” collect all (parent_dir, extension) for files under this root
    rows = (
    ...
    # Build leaf_data: parent_dir â†’ set of extensions (mirrors _collect_leaves)
    ...
    # 1b â€” exe-anchors: folders that directly contain a .exe (DB query)
    exe_anchor_rows = (
    ...
    # 2 â€” bubble up each leaf to its group root
    ...
    # 3 â€” build descriptors using DB counts (no disk reads)
    groups = []
    for root_str, exts in group_exts.items():
        grp_path = Path(root_str)
