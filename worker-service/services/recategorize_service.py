...
from diskassistent_db.models import FileRecord, RecategorizeJob, SessionLocal
...
def _run_recategorize(
    ...
    try:
        ...
        # â”€â”€ Phase 1: recategorize (skipped in regroup-only mode) â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Phase 2: regroup from DB (no disk access) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if regroup:
...
def _regroup_phase(db, job_id: int) -> None:
    ...
    entirely from the database â€” no disk scanning.
    """
    import os

    from backend.services.grouper import regroup_from_db
    from diskassistent_db.models import FileGroup, ScanJob
    ...
    if not scan_roots:
        ...
        # Find the shortest (topmost) unique paths â€” these are the scan roots
...
def _dedupe_roots(paths: list[str]) -> list[str]:
    """Keep only the topmost paths â€” remove any path that is a sub-path of another."""
    import os
