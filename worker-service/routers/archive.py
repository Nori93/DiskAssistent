...
from diskassistent_db.models import ArchiveJob, FileGroup, get_db
from services import archive_service, settings_service
...
def archive_group(group_id: int, db: Session = Depends(get_db)):
    ...
    if not archive_dir:
        raise HTTPException(400, "Archive directory is not configured.")
